...
def test_interview_scheduler_home(test_obj):
    ...
    try:
        ...
        test_obj.log_result(click_email,
                            positive="Able to pass email\n",
                            negative="Not able to pass email\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        # # Click next button 
        next_button = test_obj.click_next_button()
        test_obj.log_result(next_button,
                            positive="Able to click button\n",
                            negative="Not able to click button\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        # Enter password
        click_password = test_obj.set_password(password)
        test_obj.log_result(click_password,
                            positive="Able to pass password\n",
                            negative="Not able to pass password\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))
        ...
        test_obj.log_result(add_button,
                            positive="Able to click button\n",
                            negative="Not able to click button\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))
       
        # Enter job role
        job_text = test_obj.set_job(job)
        test_obj.log_result(job_text,
                            positive="Able to pass job role\n",
                            negative="Not able to job role\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        # Enter interviewer's name
        interviewer_text = test_obj.set_interviewer(interviewer)
        test_obj.log_result(interviewer_text,
                            positive="Able to pass interviewer's name\n",
                            negative="Not able to interviewer's name\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        # Click submit button
        submit_button = test_obj.click_submit_button()
        test_obj.log_result(submit_button,
                            positive="Able to click submit button\n",
                            negative="Not able to click submit button\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        switch = test_obj.page_switch()
        test_obj.log_result(switch,
                            positive="Able to click ok button\n",
                            negative="Not able to click ok button\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        # Click up arrow
        up_arrow = test_obj.click_up_arrow()
        test_obj.log_result(up_arrow,
                            positive="Able to click arrow\n",
                            negative="Not able to click arrow\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        # click delete link
        delete_link = test_obj.click_delete_button()
        test_obj.log_result(delete_link,
                            positive="Able to click delete button\n",
                            negative="Not able to click delete button\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        # click remove button
        remove = test_obj.click_remove_button()
        test_obj.log_result(remove,
                            positive="Able to click remove button\n",
                            negative="Not able to click remove button\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))

        # click close button
        close = test_obj.click_close_button()
        test_obj.log_result(close,
                            positive="Able to click close button\n",
                            negative="Not able to click close button\n")
        test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))
